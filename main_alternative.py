...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    stride = 1
    fconv8 = tf.layers.conv2d(vgg_layer7_out, num_classes, k_size, stride, padding='same', 
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    #upsampling 2 times from 1x1x4096 to 4x2x512
    #as upsampling is dictated by stride, adjust stride to 2
    k_size = 4  # kernal_size
    stride = 2
    upscl9 = tf.layers.conv2d_transpose(fconv8, num_classes, k_size, stride, padding='same', 
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    #skip connections by adding
    vgg_layer4_out = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, 1, padding='same', 
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    skip10 = tf.add(upscl9, vgg_layer4_out)

    #upsampling 2 times from 4x2x512 to 4x4x256
    k_size = 4  # kernal_size
    stride = 2
    upscl11 = tf.layers.conv2d_transpose(skip10, num_classes, k_size, stride, padding='same', 
                                         kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    #skip connections by adding
    vgg_layer3_out = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, 1, padding='same', 
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    skip12 = tf.add(upscl11, vgg_layer3_out)

    #upsampling 4 times from 4x4x256 to 64x64x2
    k_size = 16  # kernal_size
    stride = 8
    upscl13 = tf.layers.conv2d_transpose(skip12, num_classes, k_size, stride, padding='same', 
                                         kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    return upscl13
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    #use cross entropy to compare the difference between logits and labels
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=correct_label))

    loss_op = tf.reduce_mean(cross_entropy_loss)

    #use optimizer to minimize the difference
